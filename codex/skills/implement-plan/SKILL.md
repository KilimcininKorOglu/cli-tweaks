...
2. Research by using Grep, Glob, and file reading tools in parallel for codebase exploration
...
6. Detail each phase, then present the plan in clean markdown format
...
2. Use Grep, Glob, and file reading tools to explore the codebase:
   - Search for relevant files and understand current implementation
   - Launch multiple parallel tool calls for faster exploration
...
1. If user corrects you, research again to verify. Don't just accept.
2. Design a concrete plan based on user's chosen direction.

### Step 4: Present the Plan

Build the complete plan following the template below and present it in clean markdown format to the user for approval.
...
- Use Grep, Glob, and file reading tools for research
...
- **Search failures or empty results**: Retry with different query terms; fall back to alternative search tools; note the gap and ask user
