...
session-start.py writes the name to a lock file keyed by the Droid
...
def sessionLockFile() -> Path:
    """Return the lock file of this Droid session."""
...
def removeStaleProcessFiles(directory: Path) -> list[str]:
    """Delete the per-process files of Droid processes that no longer run.
