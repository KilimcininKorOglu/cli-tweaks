...
Usage:
  ...
  python3 compact-reinject.py AGENTS.md           # Droid
  python3 compact-reinject.py AGENTS.md AGENTS.md # Both

Looks for each file in every directory from the repository top level down to
the cwd, the same directories Droid loads project instructions from, and
also in the `.factory/` directory of each. Outside a git repository only the cwd
...
def _candidates(cwd: Path, filename: str) -> list[Path]:
    ...
    for directory in _searchDirs(cwd):
        for path in (directory / filename, directory / ".factory" / filename):
            if path.is_file():
                found.append(path)
    return found
