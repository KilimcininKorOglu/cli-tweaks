...
TEMPLATE = (
    ...
    "configuration, deployment, tests, or product preferences. Do not write global Droid "
...
if hasMemory:
    reason = (
        ...
        "under '## CRITICAL RULES' when it is a behavior rule. Do NOT write global Droid "
        ...
        "4. Write MEMORY.md and every topic file in English ONLY.\n"
        "5. Write every entry, in MEMORY.md and in every topic file, to ASD-STE100 (Simplified "
        "Technical English): short sentences, active voice, simple tenses, ONE INSTRUCTION per "
        "sentence, and the SAME term for the same thing throughout. A reason clause is not a "
        "second instruction: keep the short 'because' or 'so that' clause whenever dropping it "
        "would let the rule be applied wrongly.\n"
        "6. Name the fact directly. NEVER invent a metaphor or a figurative phrase, and NEVER "
        "present an invented phrase as if it were an established term. An invented phrase is "
        "unrecoverable months later, because the reader cannot tell which fact it encodes.\n"
        "7. NEVER hedge a fact you measured. When you have NOT verified something, say exactly "
        "that instead of softening the claim, so a guess is never read back as a fact.\n"
        "8. Reproduce identifiers, file names, config keys and trigger phrases exactly as they "
    ...
    reason = (
        ...
        "preferences. Do NOT write global Droid behavior, shared skill workflow rules, "
        ...
        "character cap (under 50000) — and write MEMORY.md and every topic file in English ONLY. "
        "Each bullet is ONE focused project rule or fact; no narrative or padding. Write every "
        "entry, in MEMORY.md and in every topic file, to ASD-STE100 (Simplified Technical "
        "English): short sentences, active voice, simple tenses, ONE INSTRUCTION per sentence, "
        "and the SAME term for the same thing throughout; a reason clause is not a second "
        "instruction, so keep the short 'because' or 'so that' clause whenever dropping it would "
        "let the rule be applied wrongly. Name the fact directly: NEVER invent a metaphor or a "
        "figurative phrase, and never present an invented phrase as if it were an established "
        "term, because an invented phrase is unrecoverable months later. NEVER hedge a fact you "
        "measured, and when you have NOT verified something say exactly that instead of softening "
        "the claim, so a guess is never read back as a fact. Reproduce "
