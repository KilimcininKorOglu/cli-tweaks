...
and **Droid-ready prompts**. The philosophy: think deeply, document thoroughly, then
...
prompt optimized for Droid execution.
...
| Prompt | `references/droid-prompt.md` | Before generating PROMPT.md |
...
#### Step 0: Scan CWD for Existing Input Documents

**Before asking any questions,** check the current working directory for pre-existing
input documents using `Glob` with these patterns:

- `prd.md`, `PRD.md`, `product-requirements.md`
- `requirements.md`, `REQUIREMENTS.md`
- `spec.md`, `SPEC.md`, `specification.md`
- `brief.md`, `project-brief.md`, `pitch.md`

If any match, read the file fully with the `Read` tool, then consult the
**"PRD → SPEC Mapping"** section in `references/elicitation-guide.md`
for how to extract elicitation answers, identify engineering-side gaps, and generate
targeted follow-up questions instead of the full Phase 0 flow. The resulting
SPECIFICATION.md must include a `Source:` line citing the input document.

If no input document is found, proceed with the full structured elicitation below.

#### Step 1: Structured Elicitation

Use the `AskUser` tool aggressively for choices — the user should tap, not type,
whenever possible.
...
Present options with trade-offs using `AskUser`.
...
**Pattern Recommendations:** Consult `references/design-patterns.md` and recommend specific
...
- Completable by Droid in a single session
...
Read `references/droid-prompt.md` before generating.
...
3. **Use `AskUser` for decisions.** Tech stack, database, auth, deployment — any
...
6. **CWD is the project root.** The skill is invoked from inside the already-created
   project folder. All output paths — both the planning docs this skill writes AND any
   directory trees the generated documents prescribe — are relative to CWD. Never create
   or assume a `project-name/` wrapper folder.
   - **Planning docs** (this skill's direct output) go to the **CWD root**, not a
     subfolder: `./SPECIFICATION.md`, `./IMPLEMENTATION.md`, `./TASKS.md`,
     `./BRANDING.md`, `./PROMPT.md`. Use the `Write` tool. This keeps the downstream
     agent's `./docs/` folder free for the project's own runtime documentation.
   - **Directory trees** inside IMPLEMENTATION.md §3.1 and PROMPT.md "Project Structure"
     MUST use `.` (CWD) as their root, NOT `project-name/` or any other wrapper.
   - **Downstream commands** prescribed in TASKS.md and PROMPT.md (e.g., `npm init`,
     `go mod init`, dependency installs) run in CWD. Never instruct the downstream
     coding agent to `cd` into a new subfolder before scaffolding.
   - The project name is only a header/title inside documents, never a directory name.
...
Check if the user has other Droid plugins or skills installed. If they do:
...
| Scenario                          | Action                                                                                                                                                                     |
|-----------------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Vague 1-liner                     | Full elicitation flow with `AskUser`                                                                                                                               |
| Detailed brief                    | Extract answers, ask only gaps                                                                                                                                             |
| `prd.md` / PRD found in CWD       | Auto-detected in Phase 0 Step 0. Read the file, apply the "PRD → SPEC Mapping" section in `elicitation-guide.md`, ask only engineering gaps, cite the source in SPEC.      |
| Existing SPECIFICATION.md uploaded | Validate against `specification-guide.md` quality checklist, suggest improvements, then continue from Phase 2.                                                            |
