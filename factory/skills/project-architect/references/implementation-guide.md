...
[Consult references/design-patterns.md. For each recommended pattern, show how it applies.]
...
> **Root = current working directory.** The project folder already exists and is the
> CWD. Do NOT wrap the tree in a `project-name/` directory. All paths below are
> relative to CWD. The planning documents (SPECIFICATION.md, IMPLEMENTATION.md,
> TASKS.md, BRANDING.md, PROMPT.md) already live at the CWD root — do not delete or
> move them. The `docs/` folder in the tree below is reserved for the project's own
> runtime documentation (API docs, architecture notes, etc.).

```
.                           # [CWD = project root; no wrapper folder]
...
├── docs/                   # [Runtime project docs: API reference, architecture, ADRs]
...
Always keep `.` as the tree root — never introduce a wrapper folder.]
