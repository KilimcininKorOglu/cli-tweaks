...
How to write PROMPT.md — a **single-shot prompt** that Droid can execute to build the
entire project from scratch. This is the most important output of the Project Architect skill.

## What Makes a Great Droid Prompt

A great prompt eliminates all ambiguity. Droid should never have to guess, infer,
...
```markdown
# [Project Name] — Droid Implementation Prompt

## Project Overview

[3-5 sentences from SPECIFICATION.md §1.1. Elevator pitch + key differentiators.
Just enough context for Droid to understand what it's building.]
...
> create a `project-name/` wrapper folder. Do NOT `cd` into a subfolder before
> scaffolding. All paths in this prompt — and all commands below — run relative to
> the current working directory. The planning documents (SPECIFICATION.md,
> IMPLEMENTATION.md, TASKS.md, BRANDING.md and this PROMPT.md) live at the CWD root
> and must be left in place — they are the source of truth for every decision in
> this prompt.

[Complete directory tree from IMPLEMENTATION.md §3.1. Every file and directory.
Root is always `.` (CWD) — never a wrapper folder.]

```
.
├── [full tree rooted at CWD]
```
...
Don't reference other documents. If Droid needs to know the database schema, put the
