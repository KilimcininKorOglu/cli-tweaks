...
Use `AskUser` tool for structured choices; reserve freeform for open-ended questions.

## Core Strategy

1. **Extract before asking.** Scan everything the user already said. Don't re-ask answered questions.
2. **Tap, don't type.** Use `AskUser` for any question with 2-4 clear options.
...
If "help me choose" → present options based on project type using `AskUser`:
```
