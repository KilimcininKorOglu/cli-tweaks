...
How to write TASKS.md — ordered, actionable work items that a developer or **Droid**
can execute sequentially to build the entire project.

## Principles

1. **Each task is a Droid session.** A task should be completable in a single focused
...
> directory. **Do NOT create or `cd` into a `project-name/` subfolder.** Every file
> path in this task is relative to CWD, and every command runs in CWD. The planning
> documents (SPECIFICATION.md, IMPLEMENTATION.md, TASKS.md, BRANDING.md, PROMPT.md)
> already live at the CWD root — leave them untouched.

**Files to create (all paths relative to CWD):**
...
**Commands to run (in CWD, no `cd` into a subfolder):**
```bash
[exact init commands: npm init -y, go mod init <module>, cargo init --name <name>, etc.]
...
- [ ] All directories listed in IMPLEMENTATION.md §3.1 exist at the top level of CWD (no wrapper folder)
...
Tasks should be written so Droid can execute them efficiently:

1. **List every file** to create or modify — Droid works best with explicit targets
2. **Include the pattern** to follow — reference IMPLEMENTATION.md section for the code pattern
3. **State the test command** — Droid should run tests after implementation
4. **Reference dependencies** — Droid needs to know what's already built
5. **Keep context self-contained** — don't say "as discussed above", repeat key context

### Example: Droid-Optimized Task
