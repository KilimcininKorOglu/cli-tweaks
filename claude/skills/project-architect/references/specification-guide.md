...
**Source:** [Traceability line. If derived from an existing input file like
`prd.md`, cite it: "Derived from `./prd.md` (read YYYY-MM-DD). Engineering gaps
filled via targeted elicitation." If authored fresh, state: "Authored from
discovery session on YYYY-MM-DD." This line stays in the final document — it
tells future readers where the requirements came from.]
