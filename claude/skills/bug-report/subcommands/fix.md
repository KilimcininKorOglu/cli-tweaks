...
Once you understand the root cause, use the agent's built-in plan mode (`ExitPlanMode`) to present a minimal fix plan: root cause in one sentence, the exact file(s) and line(s) to change, and why it is minimal (no refactoring, no scope creep).
