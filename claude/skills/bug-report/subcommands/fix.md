...
next. Only an architectural fix stops for plan-mode approval before the fix.
...
Fix directly by default. Enter Claude Code plan mode before Phase 3 ONLY when
the fix meets one of the criteria below. For a localized, non-architectural fix,
skip plan mode and proceed directly to Phase 3.
...
Enter plan mode only when you can name the criterion the fix meets. Before you
enter plan mode, write one line that names it, for example
