...
description: >-
  ...
  flows, and trust boundaries. Outputs security/architecture.md. Run this before any
  vulnerability detection skill. Use when asked to analyze a codebase for
  security or when security/architecture.md does not yet exist.
---
...
Create a `security/` folder in the project root (if it doesn't already exist). This phase produces one output file inside it:

`security/architecture.md` — technology stack, architecture, entry points, data flows
...
**Write the results of Phase 1 and Phase 2 to `security/architecture.md`.** Use this format:
