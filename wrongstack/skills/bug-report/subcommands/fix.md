...
Once you understand the root cause, use the agent's built-in plan mode (e.g. the `plan` tool or spec/plan mode) to present a minimal fix plan for THIS bug: root cause in one sentence, the exact file(s) and line(s) to change, and why it is minimal (no refactoring, no scope creep).
