---
name: goal-prep
description: Converts a free-form task description into a verifiable /goal completion condition: one measurable end state, a stated check, constraints, and an optional turn/time cap, asking focused clarifying questions only when a required component is missing. Invoked explicitly as /goal-prep.
user-invocable: true
...
Convert the user's free-form task description into a well-formed `/goal` completion **condition**: one measurable end state, a stated check, the constraints that matter, and an optional turn/time cap. Ask focused questions only when a required component is genuinely missing.
...
1. **The evaluator cannot run commands or read files.** It only sees what the agent has surfaced in the conversation transcript. So the condition must require the agent to *produce visible proof*: command output, a printed count, a diff, a status line: not just "do the thing."
...
If and only if Phase 2 found blocking gaps, ask the user once, batched (up to ~4 questions), never drip-fed across turns. Each question targets exactly one gap, with 2-4 concrete options plus room for the user's own answer.
...
- **Check**: "Agent bunu konuşmada nasıl kanıtlasın?" -> options like "`npm test` çıktısı", "`<build cmd>` exit code", "`git status` temiz", "Bir komutun stdout'u".
...
- For multi-item backlogs, make the end state a count that reaches zero or a queue that empties, and require the agent to print the count each turn.
...
3. If the work is meant to run unattended (overnight, CI, headless), say that the user should run the emitted `/goal` command through the installed WrongStack CLI's documented non-interactive mode; do not invent command syntax.
...
- ALWAYS require the agent to *surface the proof in the conversation* (print the command output, the count, the diff). The evaluator sees nothing else.
- Ask the user ONLY when a blocking gap remains, batch all questions into a single call, and offer concrete options: never interrogate one question per turn.
...
- This skill PREPARES the command; it does NOT run `/goal`. Emit the command and let the user run it. Provide the non-interactive form only when the work is meant to run unattended.
